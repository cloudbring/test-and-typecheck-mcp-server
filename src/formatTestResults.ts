...
interface FormattedTestCaseWithResult extends FormattedTestCase {
  result: TestResult;
}

export function formatTestResults(r: FormattedTestCase[]): string {
  const output: string[] = [];

  const results: FormattedTestCaseWithResult[] = r.filter(
    (t) => t.result !== undefined
  ) as FormattedTestCaseWithResult[];
  ...
  // Group tests by file
  const testsByFile = new Map<string, FormattedTestCaseWithResult[]>();
  ...
  if (failed > 0) output.push(`✗ Failed: ${failed}`);
  if (skipped > 0) output.push(`- Skipped: ${skipped}`);
  output.push("");
